...
void SysProvinceInformation::ProvinceIndustryTabs() {
    if (ImGui::BeginTabBar("ProvinceTabs", ImGuiTabBarFlags_None)) {
        ...
        if (ImGui::BeginTabItem("Economy")) {
            ...
            ImGui::TextFmt("Market GDP: {}", NumberToHumanString(market.GDP));
...
void SysProvinceInformation::DemographicsTab() {
    ...
    for (auto& seg_entity : settlement.population) {
        ...
        ImGui::TextFmt("Income: {} (per capita: {} ({} a year))",
                       NumberToHumanString(static_cast<uint64_t>(pop_segment.income)),
                       NumberToHumanString(pop_segment.income / pop_segment.population),
                       NumberToHumanString(pop_segment.income / pop_segment.population * 365));
