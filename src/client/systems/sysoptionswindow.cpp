...
void GraphicsTab(cqsp::engine::Application& app) {
    ...
    if (ImGui::BeginCombo("##window size combo box",
            fmt::format("{}x{}", common_resolutions[item_current_idx].x,
                        common_resolutions[item_current_idx].y).c_str())) {
        for (int n = 0; n < IM_ARRAYSIZE(common_resolutions); n++) {
            ...
            if (CQSPGui::DefaultSelectable(fmt::format("{}x{}", common_resolutions[n].x,
                ...
                // Change options
                app.GetClientOptions().GetOptions()["window"]["width"] = static_cast<int>(common_resolutions[n].x);
                app.GetClientOptions().GetOptions()["window"]["height"] = static_cast<int>(common_resolutions[n].y);

                if (!static_cast<bool>(app.GetClientOptions().GetOptions()["full_screen"])) {
                    // Set resolution
                    app.SetWindowDimensions(static_cast<int>(common_resolutions[n].x),
                                            static_cast<int>(common_resolutions[n].y));
                }
            }
...
void SoundTab(cqsp::engine::Application& app) {
...
void cqsp::client::systems::ShowOptionsWindow(bool* open, cqsp::engine::Application& app) {
