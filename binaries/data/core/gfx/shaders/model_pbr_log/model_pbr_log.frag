...
void main() {
    vec3 albedo = pow(texture(diffuse, TexCoords).rgb, vec3(3.));

    float roughnessValue = texture(roughness, TexCoords).r;
    vec3 N = normalize(Normal);
    float metallicValue = texture(metalness, TexCoords).r;;
