...
#include "core/util/orbit/randomorbit.h"

using ::testing::AllOf;
using ::testing::Ge;
using ::testing::Le;

using cqsp::core::components::types::Orbit;
namespace cqspt = cqsp::core::components::types;
...
TEST(OrbitTest, CircularOrbitVelocityTest) {
    // Make circular Orbit
    using namespace cqsp::core::components::types;  // NOLINT
...
TEST(OrbitTest, SolveKeplerHyperbolic) {
    ...
    for (auto &line : data) {
        EXPECT_NEAR(cqsp::core::components::types::SolveKeplerHyperbolic(std::get<0>(line), std::get<1>(line)),
                    std::get<2>(line), 1e-6);
    }
}
...
TEST(OrbitTest, SolveKeplerElliptic) {
    ...
    for (auto &line : data) {
        EXPECT_NEAR(cqsp::core::components::types::SolveKeplerElliptic(std::get<0>(line), std::get<1>(line)),
                    std::get<2>(line), 1e-6);
    }
}

TEST(core_TransferTest, TransferTimeTest_Mars) {
    namespace cqspt = cqsp::core::components::types;
...
TEST(OrbitTest, OrbitNormalTestList) {
    ...
    cqsp::core::util::OrbitGenerator generator(10, 0.5);
