...
TEST(CommandTest, SOIHierachyTest) {
    ...
    for (int i = 0; i < 5; i++) {
        ...
        auto& orbit = universe.emplace<cqsp::core::components::types::Orbit>(current_body);
        orbit.reference_body = parent_body;
        // Now make the parent
        current_body = parent_body;
        expected_hierachy.push_back(current_body);
    }
    universe.emplace<cqsp::core::components::types::Orbit>(current_body);
    // Generate a bunch of star systems

    std::vector<entt::entity> hierarchy = cqsp::core::systems::commands::GetSOIHierarchy(universe, source_body);
...
entt::entity GenerateSOITree(cqsp::core::Universe& universe, entt::entity child, int depth) {
...
TEST(CommandTest, coreSOITest) {
    // Generate a tree
    cqsp::core::Universe universe;
    entt::entity core_soi = universe.create();
    entt::entity root = GenerateSOITree(universe, core_soi, 5);
    ...
    // Now set the root to be the core soi
...
TEST(CommandTest, coreSOITestRoot) {
    ...
    cqsp::core::Universe universe;
    entt::entity core_soi = universe.create();
    universe.emplace<cqsp::core::components::types::Orbit>(core_soi);
    // Now generate the various children
    entt::entity child_1 = universe.create();
    entt::entity child_2 = universe.create();
    entt::entity root_1 = GenerateSOITree(universe, child_1, 3);
